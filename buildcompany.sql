...
DESC employees;
DESC departmentsdepartmentsemployees;
...
SELECT name AS '이름', department_id AS '부서_ID' FROM employees WHERE department_id IN (1);
SELECT name AS '이름', department_id AS '부서_ID' FROM employees WHERE department_id = 1;
...
SELECT department_id AS '부서_id', count(department_id) AS '직원 수'
	FROM employees
	GROUP BY department_id;
    
SELECT department_id AS '부서_id', count(*) AS '직원 수'
	FROM employees
	GROUP BY department_id;

-- 8. 직원 평균 나이를 계산합니다.
SELECT AVG(age) AS '평균 나이' FROM employees;

-- 9. 부서별 평균 나이를 계산합니다.
SELECT department_id AS '부서_id', AVG(age) AS '평균 나이'
	FROM employees
	GROUP BY department_id;
